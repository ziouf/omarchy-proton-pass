...
Item {
  ...
  // Non-interactive secret creation from the panel's popup form. The vault
  // and title are required; an empty password makes pass-cli generate one.
  // The payload goes to a tmpfs buffer file (same pattern as copy-value) so
  // the password never appears in process arguments.
  property bool createDone: false
  property string createError: ""

  readonly property string createBufferPath: (Quickshell.env("XDG_RUNTIME_DIR") || home + "/.cache")
                                             + "/ziouf.proton-pass.create-buffer"

  FileView {
    id: createBufferFile
    path: root.createBufferPath
    watchChanges: false
    atomicWrites: true
    printErrors: false
  }
  ...
  Process {
    ...
    stderr: SplitParser {
      onRead: function(data) {
        if (data.length <= 4096) console.warn("ziouf.proton-pass/create", truncate(data, 512).trim())
      }
    }

    onExited: function(exitCode) {
      Quickshell.execDetached(["rm", "-f", root.createBufferPath])
      if (exitCode === 0) {
        root.createDone = true
        root.createError = ""
        // Make the new item visible immediately.
        root.refreshItems(true)
      } else {
        root.createError = root.trFmt("error.createFailed", exitCode)
        root.probeSession()
      }
    }
  }

  readonly property bool createRunning: createProcess.running

  function submitCreate(vaultName, templateJson, generate) {
    if (createProcess.running) return
    root.createDone = false
    root.createError = ""
    createBufferFile.setText(templateJson)
    Qt.callLater(function() {
      var args = ["pass-cli", "item", "create", "login",
                  "--vault-name", vaultName,
                  "--from-template", root.createBufferPath]
      if (generate) args.push("--generate-password")
      createProcess.command = root.timed(root.actionDeadlineSec, args)
