...
Item {
  ...
  // and title are required; the password may be generated by pass-cli. The
  // payload goes to a tmpfs buffer file (same pattern as copy-value) so the
  // password never appears in process arguments.
  property bool createRunning: false
  ...
  Process {
    ...
    onExited: function(exitCode) {
      root.createRunning = false
      Quickshell.execDetached(["rm", "-f", root.createBufferPath])
      if (exitCode === 0) {
        root.createDone = true
        root.createError = ""
        root.notify(root.trFmt("notify.created", root.createTitle), "", "normal")
  ...
  function submitCreate(vaultName, title, templateJson) {
    if (createProcess.running) return
    root.createTitle = truncate(title, 256)
    root.createDone = false
    root.createError = ""
    root.createRunning = true
    createBufferFile.setText(templateJson)
    Qt.callLater(function() {
      createProcess.command = root.timed(root.actionDeadlineSec,
